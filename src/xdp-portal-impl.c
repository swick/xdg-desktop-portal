...
static const char ** xdp_portal_impls_get_current_desktops (XdpPortalImpls *portal_impls);
                              ...
                              gpointer      user_data)
{
  ...
  XdpPortalImpls *portal_impls = user_data;
...
find_matching_iface_config (PortalConfig *config,
...
static gboolean
portal_default_prefers_none (PortalConfig *config)
{
                               ...
                               const char     *interface)
{
  const PortalInterface *iface =
    find_matching_iface_config (portal_impls->config, interface);

  if (iface == NULL)
    return portal_default_prefers_none (portal_impls->config);
...
add_all_default_portal_implementations_iface (XdpPortalImpls *portal_impls,
                                              ...
                                              GPtrArray      *impls)
{
  const PortalInterface *iface = NULL;

  if (portal_impls->config)
    iface = portal_impls->config->default_portal;
                           ...
                           const char     *interface)
{
  ...
  iface = find_matching_iface_config (portal_impls->config, interface);
  ...
  gtk_fallback = find_gtk_fallback_portal_implementation (portal_impls, interface);
  if (gtk_fallback)
