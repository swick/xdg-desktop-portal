                 ...
                 gpointer         user_data)
{
  ...
  if (!xdp_desktop_portal_register (xdp_main->desktop_portal, connection, &error))
    {
...
main (int argc, char *argv[])
{
  ...
  g_autoptr(XdpDesktopPortal) desktop_portal = NULL;
  guint owner_id;
  g_autoptr(GError) error = NULL;
  ...
  desktop_portal = xdp_desktop_portal_new (opt_verbose);

  xdp_main.exit_status = 0;
  xdp_main.loop = loop;
  xdp_main.desktop_portal = desktop_portal;
