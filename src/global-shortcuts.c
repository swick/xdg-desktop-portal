...
void
global_shortcuts_create (XdpDesktopPortal *desktop_portal)
{
  ...
  impl = xdp_portal_impls_find (portal_impls, GLOBAL_SHORTCUTS_DBUS_IMPL_IFACE);
  if (!impl)
    {
      g_debug ("Not providing Global Shortcuts portal: No backend configured");
      return;
    }

  global_shortcuts = g_object_new (global_shortcuts_get_type (), NULL);
  global_shortcuts->impl =
    xdp_dbus_impl_global_shortcuts_proxy_new_sync (connection,
  ...
  if (!global_shortcuts->impl)
    {
      g_warning ("Not providing Global Shortcuts portal: No working backend");
      return;
    }
  ...
  g_dbus_proxy_set_default_timeout (G_DBUS_PROXY (global_shortcuts->impl), G_MAXINT);
  ...
  if (xdp_desktop_portal_export (desktop_portal,
                                 ...
                                 &error))
    {
      ...
      g_debug ("Providing Global Shortcuts portal");
    }
  else
    {
      g_warning ("Not providing Global Shortcuts portal: %s", error->message);
    }
}
